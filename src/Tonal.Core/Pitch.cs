...
public static class Pitch
{
    ...
    public static PitchInfo Get(Coordinates coord)
     {
        if (coord is PitchClassCoordinates pcCoord)
        {
        ...
        } else if (coord is IntervalCoordinates iCoord)
        {
            var f = iCoord.Fifths;
...
public record PitchInfo
{
    ...
    public PitchClassCoordinates ToCoordinates()
    {
        ...
        if (!Oct.HasValue) {
            return new PitchClassCoordinates { Fifths = (int)Dir * f };
        }

        var o = Oct.Value - Pitch.STEPS_TO_OCTS[Step] - 4 * Alt;
        return new IntervalCoordinates { Fifths = (int)newDir * f, Octaves = (int)newDir * o, Direction = newDir };
    }
}